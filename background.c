...
static const float DISTANCIA_CEU = -80.0f;      // Parede do céu (Z)
static const float DISTANCIA_FOG = -75.0f;      // Início da névoa (Z)
static const float PROFUNDIDADE_MAR = -0.15f;   // Mar logo abaixo da pista (Y)
static const float LARGURA_CENARIO = 200.0f;    // Extensão lateral
static const float ALTURA_CEU = 20.0f;          // Altura da parede (Y)

//Função para desenhar uma oval
void drawOval(float centerX, float centerY, float radiusX, float radiusY) {
    glBegin(GL_TRIANGLE_FAN);
        glVertex3f(centerX, centerY, DISTANCIA_CEU + 1.0f); // Centro da oval
        for (int i = 0; i <= 20; i++) {
            float theta = 2.0f * 3.1415926f * (float)i / 20.0f;
            // radiusX e radiusY diferentes criam o formato achatado/oval
...
void drawBackground() {
    glPushMatrix();
        // O background segue a translacao X e Z da câmera para ser infinito
        glTranslatef(cameraX, 0.0f, cameraZ);

        glDisable(GL_LIGHTING);
        glDisable(GL_DEPTH_TEST); // Garante que o fundo fique atrás de tudo
...
static void drawMountainMesh(float zMontanha) {
    glBegin(GL_TRIANGLES);
        // primeira cadeia: usa as mesmas cores da montanha direita para combinar com o estilo
        ...
        // segunda cadeia: central, mesma paleta mas mais escura no geral
        ...
        // terceira cadeia: montanha direita, esquema original mantido
        glColor3f(0.16f, 0.48f, 0.34f);
...
void drawMountain() {
    glPushMatrix();
        // Base de translação idêntica para o paralaxe infinito
        glTranslatef(cameraX, 0.0f, cameraZ);
        
        glDisable(GL_LIGHTING);
        glDisable(GL_DEPTH_TEST);

        // Montanha colada na parede do fundo (+0.5f para ficar na frente do céu)
...
void drawFog3D() {
    ...
    glBegin(GL_POLYGON);
        ...
        glVertex3f(cameraX + 31.0f,  0.5f, zFog);
        glVertex3f(cameraX - 31.0f,  0.2f, zFog);
        
        glColor4f(1.0f, 1.0f, 1.0f, 0.0f); 
        glVertex3f(cameraX - 31.0f, -4.0f - 1.0f, zFog);
        glVertex3f(cameraX + 31.0f, -4.0f - 1.0f, zFog);
    glEnd();
    ...
    glBegin(GL_POLYGON);
        ...
        glVertex3f(cameraX + 31.0f,  -0.1f, zFog);
        glVertex3f(cameraX - 31.0f,  -0.1f, zFog);
